...
bool ParseApfsBlockDevice(BlockDeviceInfo* info) {
    ...
    for (apfs_vol = 0; apfs_vol < 20; ++i) {
        errno = 0;
        mountopts = "vol=" + std::to_string(i);
        ret = mount(devpath.c_str(), kTryMountTarget.c_str(), try_fs_type.c_str(), mountflags, mountopts);
        save_errno = errno;
        if (ret) {
    ...
    info->mountopts = "vol=" + std::to_string(i);
    info->rw = false;
...
void ParseBlockDevice(BlockDeviceInfo* info) {
    ...
    info->mountopts = bdinfo->rw ? kMountOptsRw : kMountOptsRo;
...
void OnPostBlockDevices(void) {
    ...
    if (need_android_dir) {
        ...
        ret = mount(bdinfo->devpath.c_str(), kAndroidMountTarget.c_str(), bdinfo->fs_type.c_str(), bdinfo->rw ? 0 : MS_RDONLY, bdinfo->mountopts);
        if (ret) {
    ...
    if (need_firmware_dir) {
        ...
        if (bdinfo == block_device_for_android_dir) {
            ...
            ret = mount(bdinfo->devpath.c_str(), kFirmwareMountTarget.c_str(), bdinfo->fs_type.c_str(), MS_RDONLY, bdinfo->mountopts);
            if (ret) {
