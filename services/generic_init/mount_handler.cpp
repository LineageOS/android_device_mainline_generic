...
bool ParseApfsBlockDevice(BlockDeviceInfo* info) {
    ...
    for (apfs_vol = 0; apfs_vol < 20; ++apfs_vol) {
        ...
        ret = mount(devpath.c_str(), kTryMountTarget.c_str(), "apfs", mountflags, mountopts);
        save_errno = errno;
        if (ret) {
...
void OnPostBlockDevices(void) {
    ...
    if (need_android_dir) {
        ...
        ret = mount(bdinfo->devpath.c_str(), kAndroidMountTarget.c_str(), bdinfo->fs_type.c_str(), bdinfo->rw ? 0 : MS_RDONLY, std::to_string(bdinfo->mountopts));
        if (ret) {
    ...
    if (need_firmware_dir) {
        ...
        if (bdinfo == block_device_for_android_dir) {
            ...
            ret = mount(bdinfo->devpath.c_str(), kFirmwareMountTarget.c_str(), bdinfo->fs_type.c_str(), MS_RDONLY, std::to_string(bdinfo->mountopts));
            if (ret) {
