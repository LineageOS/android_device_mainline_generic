...
Ramdisk could be a merge of multiple ramdisk images in the same format.

Ramdisk contains a init program, possibly with kernel modules and firmwares too.
...
and a mount handler that replaces `first_stage_mount.cpp` that automatically scans partitions
...
does not really differ from the one in AOSP `/init`.

### Loading basic kernel modules in ramdisk

Kernel modules that are required to probe the boot media, and the kernel modules for basic SoC platform support,
...
and the modules with prefix listed on `modules.load_prefix`.
...
(with the entries on the specified addon fstab included), finally pass the fstab to
`MountHelpers::MountPartitions()` which is basically a copy from
AOSP `first_stage_mount.cpp`, and let it finally the reset of mounts.
...
It calculates PPI (Pixel-Per-Inch) value from the EDID from the first connected connector
