...
Create a `.mk` file in the source tree for the place to put kernel build configuration on, and specify the path to the file by this command:

```
export MAINLINE_GENERIC_KERNEL_BOARDCONFIG_MK=<path to the file>
```
...
By default, we decide which kernel modules to be installed in ramdisk using approach like Debian's `initramfs-tools`, which is based on the module's source directory.
You can also specify to install and load some specific modules in ramdisk, by editing `configs/kernel/boot_kernel_modules_finder.sh` and
`configs/modprobe/modules.load.ramdisk` in this device tree.
...
`configs/modprobe/modules.load` and `configs/modprobe/modules.load_prefix` in this device tree.

## Make kernel modules load options file

If the target device needs to load kernel modules with specific options, do this.
...
Kernel modules loaded in ramdisk stage may request for firmware files, and such of firmware files should be included in ramdisk.
