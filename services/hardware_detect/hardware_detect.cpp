...
void DetectGraphics(void) {
    ...
    if (kMustUseFbDisplayCards.find(card_name) != kMustUseFbDisplayCards.end()) {
        ...
        // nothing
    } else if (card_name == "asahi") {
        // Not checking with "apple" here, because it seems like it appears when simpledrm is enabled, and does not work with 3D graphics
        // nothing
    ...
    if (render_fd < 0) {
        ...
        DrmAmdgpuRender();
    } else if (render_name == "asahi" || render_name == "apple") {
        DrmAsahiRender();
